...
#define AUTOSHUTDOWN_PIN 5//auto shutdown enabled when pin connected to GND
...
volatile uint8_t audiosense_learn_detected = 0;
...
void setup(){
  ...
  pinMode(AUTOSHUTDOWN_PIN, INPUT_PULLUP);
...
void on_audio_enabled_exit() {
  ...
  //send AUDIO STOP IR signal but only when auto shutdown enabled (connected to GND)
  if(digitalRead(AUTOSHUTDOWN_PIN) == LOW) sendCode(0, stopAudioCodeType, stopAudioCodeLen, stopAudioCodeValue); //send recorded or saved IR code without repeat
